...
/* Global seed variable for the rndm number generator.  */
static unsigned long long global_seed;

/* Simple rndm number generator.  The numbers are in the range from
...
/* Fill LENGTH bytes at BUFFER with rndm contents, as determined by
   SEED.  */
static void
...
initial_allocations (void)
{
  ...
  /* Execute the tasks in a rndm order.  */
...
do_test (void)
{
  ...
  /* Execute the post-undump tasks in a rndm order.  */
  shuffle_allocation_tasks ();
