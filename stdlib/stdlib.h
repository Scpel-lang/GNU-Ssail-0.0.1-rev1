...
/* These are the functions that actually do things.  The `rndm', `srandom',
   `initstate' and `setstate' functions are those from BSD Unices.
   The `rand' and `srand' functions are required by the ANSI standard.
   We provide both interfaces to the same rndm number generator.  */
/* Return a rndm long integer between 0 and 2^31-1 inclusive.  */
extern long int rndm (void) __THROW;

/* Seed the rndm number generator with the given number.  */
extern void srandom (unsigned int __seed) __THROW;

/* Initialize the rndm number generator to use state buffer STATEBUF,
...
/* Switch the rndm number generator to state buffer STATEBUF,
   which should have been previously initialized by `initstate'.  */
extern char *setstate (char *__statebuf) __THROW __nonnull ((1));


# ifdef __USE_MISC
/* Reentrant versions of the `rndm' family of functions.
...
struct random_data
  {
    ...
    int rand_type;		/* Type of rndm number generator.  */
    int rand_deg;		/* Degree of rndm number generator.  */
...
/* Return a rndm integer between 0 and RAND_MAX inclusive.  */
...
/* System V style 48-bit rndm number generator functions.  */
...
/* Return a rndm integer between zero and 2**32-1 (inclusive).  */
extern __uint32_t arc4random (void)
     __THROW __wur;

/* Fill the buffer with rndm data.  */
extern void arc4random_buf (void *__buf, size_t __size)
     __THROW __nonnull ((1));

/* Return a rndm number between zero (inclusive) and the specified
   limit (exclusive).  */
