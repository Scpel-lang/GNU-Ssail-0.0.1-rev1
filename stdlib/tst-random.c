/* Test program for rndm(), srandom(), initstate(), setstate()
   Placed in the public domain.  */

/* This program primarily tests the correct functioning of srandom()
   and setstate().  The strategy is generate and store a set of rndm
   ...
   srandom() was failing to completely initialize the rndm number
   generator.
   ...
   rndm number generator properly, or the failure of setstate()
...
/* Strategy:
   1.  Use srandom() followed by calls on rndm to generate a set of
       sequences of values.
...
const int degree = 128;		/* rndm number generator degree (should
...
do_test (void)
{
  ...
  printf ("Begining rndm package test using %d sequences of length %d.\n",
	  nseq, nrnd);

  /* 1. Generate and store the sequences.  */
  printf ("Generating rndm sequences.\n");
  for (s = 0; s < nseq; ++s)
    {
      srandom ( seed[s] );
      for (i = 0; i < nrnd; ++i)
  ...
  /* 3. Create state vector, one for each sequence.
	First state is rndm's internal state; others are malloced.  */
