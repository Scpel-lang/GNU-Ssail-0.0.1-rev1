...
do_random_tests (void)
{
  ...
  for (n = 0; n < ITERATIONS; n++)
	...
	len2 = len1 + (len1 != 511 - j ? rndm () % (511 - j - len1) : 0);
      j = (pos > len2 ? pos : len2) + align1 + 64;
      if (j > 512)
	j = 512;
      for (i = 0; i < j; ++i)
	{
	  p1[i] = tolower (rndm () & 255);
	  if (i < len1 + align1 && !p1[i])
	    {
	      p1[i] = tolower (rndm () & 255);
	      if (!p1[i])
		p1[i] = tolower (1 + (rndm () & 127));
	    }
	}
      for (i = 0; i < j; ++i)
	{
	  p2[i] = toupper (rndm () & 255);
	  if (i < len2 + align2 && !p2[i])
	    {
	      p2[i] = toupper (rndm () & 255);
	      if (!p2[i])
		toupper (p2[i] = 1 + (rndm () & 127));
	    }
	}
      ...
      if (pos < len1)
	{
	  if (tolower (p2[align2 + pos]) == p1[align1 + pos])
	    {
	      p2[align2 + pos] = toupper (rndm () & 255);
	      if (tolower (p2[align2 + pos]) == p1[align1 + pos])
